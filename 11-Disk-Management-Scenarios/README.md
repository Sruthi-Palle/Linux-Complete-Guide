...
Choose this when your primary drive (where the OS and Docker live) is full. You increase the size in the AWS Console, but the Linux OS needs to be told to "claim" that extra space.
![alt text](../Images/scenario-1.png)

#### **The Workflow**

1.  **AWS Console:** Change Volume Size (e.g., 8GB → 30GB).
2.  **Expand Partition:** Tell the partition table to grow into the new space.

    ```bash
    sudo growpart /dev/xvda 1
    ```

    _(Note: If your device is NVMe, it might be_ `/dev/nvme0n1` _and partition_ `1`_)_

3.  **Expand File System:** Stretch the actual filesystem to fill the partition.
    - **For EXT4:** `sudo resize2fs /dev/xvda1`
    - **For XFS:** `sudo xfs_growfs -d /`

      \*Note: This preserves your existing data. No formatting required.

---

### **Scenario 2: Adding a New Volume (Secondary Storage)**
...
1.  **Verify the new device:** Use `lsblk` to see the new disk. It will likely appear as `xvdf` or `nvme1n1` with no partitions.

    > _Look for the disk with no "MOUNTPOINT" and the size you just created (e.g.,_ `nvme1n1` _or_ `xvdf`_)._

2.  **Safety Check:** Confirm the disk is empty.

    ```bash
    sudo file -s /dev/xvdf
    ```

    > _If it says "data", it’s safe to format. If it shows a filesystem,_ **_skip formatting._**  
    > Never format a disk that already has a file system.

3.  **Format the raw storage/blank disk:** Create the filesystem structure.
...
4.  **Mounting:** Create a directory and link the hardware to it.
...
Without this, your disk will unmount after a reboot.In this **Scenario 2,** if you reboot the server, the disk will **disappear**. To make it stay, you must add it to `/etc/fstab`:

1.  Get the UUID: `sudo blkid /dev/xvdf`
2.  Add to `/etc/fstab`: `UUID=your-uuid-here /mnt/data ext4 defaults,nofail 0 2`
